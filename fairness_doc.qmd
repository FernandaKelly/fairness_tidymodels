...
Podemos constatar que pessoas consideradas **caucaseanas** e **afro americanas** são quase **93%** de toda a população da base de dados e que **75%** é composto por apenas por pessoas **caucasianas**. 
...
Observe as porcentagens onde houve readmissão. O que podemos inferir a questão de raça? É uma questão a se questionar.

```{r}
library(janitor)

dataFairness %>% 
   janitor::tabyl(readmitted, show_na = TRUE)
```
