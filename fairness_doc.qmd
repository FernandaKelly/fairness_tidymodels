...
Com a função **initial_split** temos dois parâmetros para codificar a proporção (o mais utilizado) ou o estrato da reamostragem, são eles:

-   prop: A proporção de dados a serem retidos para modelagem/análise.
...
Neste caso, vamos utilizar a opção de **strata** com a variável **readmitted** com o objetivo de abordar o desequilíbrio de classe, visto que somente **8%** do banco de dados tiveram readmissão. Para saber mais sobre desequilíbrios de classe e estratificação, consulte o capítulo “Métodos comuns para divisão de dados” de Modelagem organizada com R (Kuhn e Silge 2022) que é incrível e vai te ajudar a entender a escolha da utilização dessa reamostragem.

```{r}
set.seed(1)

readmission_splits <- rsample::initial_split(dataFairness, strata = readmitted)
